...
6. 🗣️ استخدمي اللغة العربية الطبيعية والودودة\n\n`;
  ...
  async buildAdvancedPrompt(customerMessage, customerData, companyPrompts, ragData, conversationMemory , hasImages = false, smartResponseInfo , messageData ) {
    ...
    // Add response guidelines - Natural and flexible
    if (companyPrompts.responsePrompt) {
      prompt += `${companyPrompts.responsePrompt}\n\n`;
    } else {
      prompt += `📋 إرشادات الرد:
...
- 🎯 مهم جداً: لو العميل طلب أكتر من منتج (مثلاً "عايز أشوف المنتج أ والمنتج ب")، ابعتي صور كل منتج على حدة - ابدأي بالمنتج الأول وبعدين المنتج التاني\n\n`;
    }
...
${isNewCustomer ? '💡 ملاحظة: العميل ده جديد - اهتمي بيه وخليه يحس بالترحيب والاهتمام!\n' : '💡 ملاحظة: العميل ده راجع - حافظي على نفس مستوى الخدمة الممتازة!\n'}\n`;
    ...
    // Add contextual guidance based on conversation stage
    if (ragData && ragData.length > 0) {
      // 🆕 فحص إذا كان هناك منتجات متعددة
      const multipleProductsFound = smartResponseInfo?.multipleProducts && smartResponseInfo.multipleProducts.length > 1;
      
      prompt += `💡 ملاحظات مهمة:
- المعلومات أعلاه هي المتاحة حالياً - استخدميها بشكل طبيعي في ردك
- ${hasImages ? (multipleProductsFound ? `الصور متاحة وستُرسل تلقائياً لكل المنتجات المطلوبة (${smartResponseInfo.multipleProducts.length} منتج)` : 'الصور متاحة وستُرسل تلقائياً مع ردك') : 'الصور غير متاحة حالياً'}
- كوني طبيعية ومرنة في الرد - تكلمي كأنك بتساعدي صديق
${multipleProductsFound ? `- 🎯 مهم جداً: العميل طلب ${smartResponseInfo.multipleProducts.length} منتجات وكلهم متوفرين! هبعتلك صور كل منتج. اذكري كل المنتجات اللي لقيتيها في ردك` : ''}
${smartResponseInfo && smartResponseInfo.hasSpecificProduct && !multipleProductsFound ? `- العميل يسأل عن: ${smartResponseInfo.productInfo.product?.metadata?.name}` : ''}\n\n`;
    } else {
      prompt += `💡 ملاحظة: لا توجد معلومات محددة في قاعدة البيانات - ساعدي العميل بطريقة ودودة واسأليه عن التفاصيل\n\n`;
    }
...
`;
  ...
  async checkDataCompleteness(orderDetails, conversationMemory, messageContent ) {
    ...
    let conversationText = conversationMemory.map(m => `${m.userMessage || ''} ${m.aiResponse || ''}`).join(' ');
...
async extractOrderDetailsFromMemory(conversationMemory, companyId, currentMessage) {
  ...
  buildConversationContext(conversationMemory) {
    ...
    recentMessages.slice(0, 3).forEach((msg, i) => {
      console.log(`  [${i}] userMessage: "${msg.userMessage?.substring(0, 50) || 'N/A'}", aiResponse: "${msg.aiResponse?.substring(0, 50) || 'N/A'}"`);
    });

    const contextText = recentMessages.map((interaction, index) => {
      // ✅ استخدام userMessage و aiResponse من الذاكرة
      const userMsg = interaction.userMessage || '';
      const aiMsg = interaction.aiResponse || '';
      const timestamp = interaction.timestamp ? new Date(interaction.timestamp).toLocaleTimeString('ar-EG') : '';

      let text = '';
      if (userMsg) {
        text += `[${index + 1}] ${timestamp} العميل: ${userMsg}`;
      }
      if (aiMsg) {
        text += `\n[${index + 1}] ${timestamp} النظام: ${aiMsg}`;
      }
      return text;
    }).filter(t => t).join('\n\n');
      ...
      if (wantsImages) {
        ...
        if (refersToMultiple && conversationMemory && conversationMemory.length > 0) {
          ...
          if (uniqueProductNames.length >= 2) {
            ...
            for (const productName of uniqueProductNames) {
              ...
              const contextualRequest = `عايز أشوف ${productName}`;
              const specificResult = await ragService.retrieveSpecificProduct(contextualRequest, intent, customerId, conversationMemory, companyId);
        ...
        if (hasMultipleProducts) {
          ...
          for (const productRequest of productRequests) {
            ...
            // إضافة "عايز أشوف" للسياق لجميع المنتجات في حالة الطلبات المتعددة
            // هذا يساعد في البحث الدقيق عن كل منتج
            const contextualRequest = `عايز أشوف ${trimmedRequest}`;
            
            console.log(`🔍 [SMART-RESPONSE] الطلب مع السياق: "${contextualRequest}"`);
            const specificResult = await ragService.retrieveSpecificProduct(contextualRequest, intent, customerId, conversationMemory, companyId);
        ...
        // استخدام النظام الذكي للمنتجات (منتج واحد)
