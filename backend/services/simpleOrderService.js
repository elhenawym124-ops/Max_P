...
class SimpleOrderService {
  ...
  async createSimpleOrder(data) {
    try {
      ...
      // ✅ استخدام رقم الطلب المُمرر أو إنشاء رقم جديد
      const orderNumber = data.orderNumber || `ORD-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
